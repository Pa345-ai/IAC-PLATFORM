...
class Proof:
    ...
    def generate(self) -> Dict[str, Any]:
        ...
        # Per-layer accumulators for correct weighted averaging
        layer_scores  = {"intent": [], "execution": [], "remediation": []}
        total_records = 0

        if not os.path.exists(input_dir):
            logging.error(f"Input path {input_dir} missing.")
            return self._finalize(False, 0.0, output_file)
        ...
        # FIX 1: Correct weighted average — average per layer, then weight
        combined_score = 0.0
        for layer, cfg in self.thresholds.items():
            ...
            # Missing layer contributes 0 for its weight (penalises omission)

        # Scale multiplier — logarithmic, same as before
