...
# Global IAM Configuration with Permission Boundaries
...
################################################################################
# Permission Boundary Policy (CRITICAL)
################################################################################

resource "aws_iam_policy" "permission_boundary" {
  name        = "BankPermissionBoundary"
  description = "Maximum permissions allowed for all IAM entities (PCI-DSS 7.2.1)"
  path        = "/"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid    = "DenyModifyPermissionBoundary"
        Effect = "Deny"
        Action = [
        ...
        Condition = {
          StringNotEquals = {
            "aws:PrincipalArn" = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/SecurityAdmin"
          }
        }
      },
      {
...
resource "aws_iam_role" "admin" {
  name                 = "AdminRole"
  description          = "Emergency admin role - requires MFA"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringEquals = {
            "sts:ExternalId" = var.admin_external_id
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "developer_write" {
  name = "DeveloperWriteAccess"
...
################################################################################
# IAM Password Policy (PCI-DSS 8.2)
################################################################################
...
resource "aws_accessanalyzer_analyzer" "main" {
  ...
  tags = {
    Name    = "OrganizationAccessAnalyzer"
