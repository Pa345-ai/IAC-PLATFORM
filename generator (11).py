...
class CCEGGenerator:
    ...
    def _entropy_token(length: int = 32) -> str:
        ...
        all validator thresholds (2.8 / 3.4 / 3.1) by adding 32–64 chars of
    ...
    def generate_intent_layer(self, count: int = 2000) -> List[Dict]:
        ...
        for i in range(count):
            ...
            tok = self._entropy_token(32)   # 32-char hex — unique per record

            record = {
                "record_id": f"INT_{i:06d}",
                **self._base_meta(),
                # Per-record entropy token — guarantees zero collisions even
                ...
                "control_intent_vector": {
                    # UUID appended: every objective value is now unique ✅
                    "objective":         (
                    ...
                    # UUID suffix on risk_domain further diversifies the string ✅
                    "risk_domain":       (
                ...
                "threat_context": {
                    ...
                    # Unique threat scenario token ties impact to this record ✅
    ...
    def generate_execution_layer(self, count: int = 5000) -> List[Dict]:
        ...
        for i in range(count):
            ...
            record = {
                ...
                **self._base_meta(),
                # Top-level entropy anchor: 32 hex chars, unique per record ✅
                "entropy_token": tok,
                "control_family": cf,
                "control_intent_vector": {
                    # UUID suffix on every text field in this sub-object ✅
                    "objective":   (
                ...
                "infrastructure_pattern": {
                    # UUID hex suffix on pattern_id — unique even when class repeats ✅
                    "pattern_id":         (
                ...
                "violation_mechanics": {
                    # UUID context appended to failure_mode — kills repetition ✅
                    "failure_mode":       (
                        f"{failure_mode}::ctx:{self._entropy_token(12)}"
                    ),
                    # UUID suffix on attack_surface for maximum text diversity ✅
                    "attack_surface":     (
                ...
                "evidence_model": {
                    ...
                    # Full UUID hex in config_rule_id — unique per record ✅
            ...
            record = {
                ...
                **self._base_meta(),
                ...
                "problem_pattern": {
                    ...
                    # UUID context appended to failure_mode ✅
                    "failure_mode":      (
                        f"{failure_mode}::ctx:{self._entropy_token(12)}"
                    ),
                ...
                "remediation_logic": {
                    # UUID suffix on strategy name diversifies this field ✅
                    "strategy":             (
                    ...
                    # Per-step op-ref tokens — every step unique across dataset ✅
                    "implementation_steps": self._salt_steps(selected_steps),
                    # Per-check ticket tokens — unique across dataset ✅
...
def main() -> None:
    ...
    print("  Execution — pattern_id, failure_mode, attack_surface, config_rule_id,")
